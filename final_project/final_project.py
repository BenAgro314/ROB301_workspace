...
from vision import (BLUE, COLOR_CODES, COLOR_TO_NAME, GREEN, LINE, RED, YELLOW,
                    decode_px, get_color_prob, is_on_line)
...
class BayesLoc:
    ...
    def step(self):
        if self.current_control is None: # we have not yet executed a contro
            ...
            #on_line = is_on_line(self.curr_px, self.curr_color)
            #assert not on_line, "WE SHOULDN'T BE ON THE LINE NOW"
    ...
    def go_to_rooms(self, threshold = 0.90):
        ...
        if len(self.to_visit) > 0:
            if self.current_control is None or not self.current_control.is_alive(): # we have finished executing last control, check our probability
                ...
                print(most_likely_room, max_prob)
                print(self.to_visit)
                print("Control qeue", [str(s) for s in self.control_queue])
                if max_prob > threshold and most_likely_room in self.to_visit:
                    
                    self.control_queue.append(Control(0)) # stop
                    #self.control_queue.append(Control(1)) # then move forwards
                    self.to_visit.remove(most_likely_room)
                else:
                    self.control_queue.append(Control(1)) # move forwards and wait for convergence
        self.execute_control()
    ...
    def measurement_model(self):
        ...
        for i in range(len(self.color_map)):
            prob[i][0] = get_color_prob(self.color_map[i], self.curr_color)
            
        # we return an 11x1 numpy array where the ith row is
        # p(z_k = ROOM COLOR[row] | self.curr_color)
        # e.g if room 2 is yellow first entry is p(z_k = yellow| current rgb value)
        return prob
...
if __name__ == "__main__":
    ...
    # TODO calibrate these RGB values to recognize when you see a color
    # NOTE: you may find it easier to compare color readings using a different
    # color system, such as HSV (hue, saturation, value). To convert RGB to
    # HSV, use:
    ...
    #localizer.control_queue = [Control(1) for i in range(12)] #,Control(1),Control(1)] # forward 4 offices
